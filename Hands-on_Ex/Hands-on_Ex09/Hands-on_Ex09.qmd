...
# Hands-on Exercise 9 - Information Dashboard Design: R methods
...
As of March 2026, the `reactablefmtr` package was archived and removed from the official CRAN repository network. Because R can't find the package on the main servers anymore, it throws up its hands and assumes the package just isn't built for your version of R.

Since we can't get it from CRAN right now, we have to go straight to the source and download it directly from the creator's GitHub. As such you would need to run the following codes:
