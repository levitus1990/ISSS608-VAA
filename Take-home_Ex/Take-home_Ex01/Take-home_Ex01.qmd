...
**Interpretation:** The variable with the higher absolute Spearman ρ and a tighter, more monotonically declining GAM curve is the superior risk predictor. If `years_licensed` produces a cleaner declining curve than `driver_age`, this provides actuarial evidence that driving experience is a better explanatory variable for claim frequency than chronological age — a potentially significant finding for the pricing model, since the two variables are correlated but not identical, particularly for late-licensed drivers.
...
**Interpretation:** If the frequency panel shows luxury brand drivers claiming less often while the severity panel shows them incurring substantially higher costs per claim, this confirms the "prestige paradox." This has direct pricing implications: luxury vehicles should carry reduced frequency loadings but substantially elevated severity loadings — a structural rating model refinement that a combined loss ratio comparison would completely miss.
...
**Interpretation:** A significant chi-square result here confirms that brand tier is an adverse selection signal independent of all other rating factors — that policyholders of certain brand tiers are systematically concentrated in specific bonus score bands. If luxury brand holders carry a disproportionate share of "Bad" bonus scores, brand tier deserves its own loading factor in the rating engine, not just a value-based severity adjustment. This finding would be novel: brand tier is rarely used explicitly in motor insurance pricing despite being a proxy for driver risk-taking behaviour.
...
**Interpretation:** Comparing the GAM curve slope and confidence band width across the three panels reveals whether the power-to-weight proxy adds independent predictive signal beyond the brand tier classification. A monotonically increasing curve in the mass-market panel with a flat or ambiguous curve in the luxury panel would confirm this variable as a targeted rating factor for the non-luxury book — a commercially significant finding that could be incorporated into the underwriting rating engine for accessible performance vehicles.
...
**Interpretation:** If the glass-claimer density curve (red) shows a heavier right tail — elevated density at claim frequencies above 1.0 — relative to the non-claimer curve (blue), this confirms glass claims are a behavioural leading indicator of future multi-claim events. This would provide actuarial justification for incorporating glass claims history into the bonus-malus scoring system, which typically treats glass claims as minor administrative events. Even a modest right-tail difference, if statistically confirmed, would support a small surcharge on renewal policies where glass claims occurred in the prior year.
...
**Interpretation:** The retention paradox would be confirmed if: (1) Good-score policies cancel at higher rates than Bad-score policies (from the first chart), *and* (2) within the Good-score panel, the cancelled density peaks to the left of the active density (the cancelled policies had lower loss ratios — they were the better risks). The commercial prescription is direct: retention pricing for Good-score segments must be competitive with the open market, while Bad-score retention should be managed through underwriting tightening rather than price concessions.
...
**Interpretation:** If short-duration policies show right-shifted loss ratio distributions — higher median, heavier right tail — compared to full-duration policies, this confirms that exposure duration is a quality signal rather than merely a volume variable. Policies that terminate early are disproportionately associated with claims followed by cancellation, suggesting that requiring minimum contract terms or applying early-termination loadings for comprehensive cover would reduce this source of adverse selection.
