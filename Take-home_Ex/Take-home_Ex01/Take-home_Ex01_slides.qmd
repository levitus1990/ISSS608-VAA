...
**What this tells the business:** The island segment, comprising around 24,000 policies, shows much wider uncertainty bands in the loss ratio analysis (Slide 11) than other segments, reflecting greater variability in loss outcomes within that segment. Further analyses for pricing in respect of Island drivers might be warranted to account for the higher variability. Most insured vehicles are 20–25 years old on average, and the typical vehicle is worth approximately €24,700.
:::
...
**Key business insights from univariate analyses:** An older vehicle fleet + middle-aged driver base means demographic extremes are bypassed. Risk concentration is instead occurring in the **product and acquisition channel dimensions** (COMP_N adverse selection, NB vs Portfolio gap).
:::
:::::
...
geo_ci <- insurance_viz |>
  ...
  mutate(
    ...
    geo_seg = fct_reorder(geo_seg, loss_ratio, median)
  )
ggplot(geo_ci, aes(x = geo_seg, y = loss_ratio)) +
  ...
  labs(title = "Loss Ratio by Geographic Segment",
       subtitle = "Urban > Rural across all types; wide island CIs = unreliable estimates",
       x = NULL, y = "Loss Ratio (log)") +
...
::: {style="font-size:0.75em; margin-top:4px; padding:5px 12px; background:#fef9e7; border-left:3px solid #c0392b; border-radius:3px;"}
**What this tells the business:** **Bad-score policyholders are the stickiest cohort**. While they cancel more (25% vs. 10% for Good), **75% remain active**, representing the largest absolute risk retention because they likely lack external options. 

On geography, median loss ratios are similar across urban and rural segments within each municipality type. This chart alone does not confirm circulation area as a meaningful risk discriminator, and a formal statistical test would be needed before treating it as a rating factor. Island segments present a more pressing concern: their confidence intervals are dramatically wider than mainland segments, meaning the observed median is an unreliable pricing input. The insurer should apply an explicit uncertainty loading for island policies rather than pricing off the median alone.
:::
